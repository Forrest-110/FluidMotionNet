...
from tools.losses import chamfer_loss, smooth_loss,divergence_loss_direct_Version
from models.lattice import Lattice
# from models.neural_prior import Neural_Prior
...
class Refiner(torch.nn.Module):
    def __init__(self, shape, device):
        ...
        self.div_criterion=Lattice(nb=8,spacing=10,batchsize=shape[0],device=device)
    ...
    def refine_flow(self, batch, flow, corr_conf, optimizer, args):
        ...
        div_flow_loss_all = np.zeros(args.test_time_num_step + 1, dtype=np.float32)
        ...
        for step in range(args.test_time_num_step):
            ...
            if args.use_div_flow and args.div_flow_loss_weight > 0:
                # div_flow_loss = divergence_loss_direct_Version(pc_0_orig, refined_flow, nb_neigh=args.nb_neigh_div_flow)
                div_flow_loss = self.div_criterion(refined_flow,pc_0_orig)
            ...
            loss.backward()
            optimizer.step()
            # print the gradient of the refinement
            # print(self.refinement.grad.isnan().any())
            ...
            div_flow_loss_curr = div_flow_loss.item() if args.use_div_flow and args.div_flow_loss_weight > 0 else div_flow_loss
            ...
            if args.test_time_verbose:
                print("Refinement step %04d/%04d: loss: %.6f, target_recon_loss: %.6f, smooth_flow_loss: %.6f, div_flow_loss: %.6f, epe: %.3f" %
                      ((step + 1), args.test_time_num_step, loss_curr, target_recon_loss_curr, smooth_flow_loss_curr, div_flow_loss_curr, epe_curr))

            # aggregate results
            target_recon_loss_all[step] = target_recon_loss_curr
            smooth_flow_loss_all[step] = smooth_flow_loss_curr
            div_flow_loss_all[step] = div_flow_loss_curr
        ...
        refine_metrics = {"target_recon_loss_all": target_recon_loss_all, "smooth_flow_loss_all": smooth_flow_loss_all, "div_flow_loss_all": div_flow_loss_all , "epe_all": epe_all}
...
# class PriorRefiner(torch.nn.Module):
#     def __init__(self, shape, device):
#         """
#         Construct a model for refining scene flow between two point clouds.
...
#         self.net=Neural_Prior().to(device)
#         for param in self.net.parameters():
#             param.requires_grad = True
#         self.net_inv=copy.deepcopy(self.net)
#         for param in self.net_inv.parameters():
#             param.requires_grad = True
#         self.params=[{'params': self.net.parameters()},
#                 {'params': self.net_inv.parameters()}]
