...
def compute_flow(model, batch, args):
    ...
    with torch.no_grad():
        ...
        feats_0, graph = model.extractor(pc_0)
        feats_1, _ = model.extractor(pc_1)
        ...
        for b in iterate_in_chunks(idx, args.nb_points_chunk):
            ...
            points_flow, points_conf, _,similarity_cross = model.get_recon_flow([points, pc_1_orig], [feats, feats_1_orig])
            est_flow[:, b] = points_flow
            corr_conf[:, b] = points_conf

    return est_flow, corr_conf, graph,similarity_cross
...
def compute_epe_test(est_flow, batch, args):
    ...
    est_flow : torch.Tensor
        Estimated flow.
    batch : model.datasets.generic.Batch
    ...
    nepe3d_per_point = np.linalg.norm(sf_gt - sf_pred, axis=-1)/np.linalg.norm(sf_gt,axis=-1)
    epe3d = epe3d_per_point.mean()
    nepe3d = nepe3d_per_point.mean()
    ...
    return epe3d, acc3d_strict, acc3d_relax, outlier, epe3d_per_point, acc3d_strict_per_point, acc3d_relax_per_point, outlier_per_point,nepe3d


def eval_model(model, testloader, log_file, log_dir, res_dir, args):
    ...
    nepe_per_scene=np.zeros(num_batch, dtype=np.float32)
    ...
    if args.use_test_time_refinement:
        ...
        div_flow_loss_refinement = np.zeros([num_batch, args.test_time_num_step + 1], dtype=np.float32)
    ...
    for it, batch in enumerate(tqdm(testloader)):
        ...
        if args.all_candidates:
            est_flow, corr_conf, graph,similarity_cross = compute_flow(model, batch, args)
        else:
            with torch.no_grad():
                est_flow, corr_conf, _, graph = model(batch["sequence"])
        ...
        if args.use_test_time_refinement:
            ...
            div_flow_loss_refinement[it] = refine_metrics_curr["div_flow_loss_all"]
        ...
        # Performance metrics
        epe3d, acc3d_strict, acc3d_relax, outlier, epe3d_pp, acc3d_strict_pp, acc3d_relax_pp, outlier_pp ,nepe3d =\
            compute_epe_test(est_flow, batch, args)
        epe_per_scene[it] = epe3d
        nepe_per_scene[it] =nepe3d
    ...
    mean_nepe =running_nepe / num_batch
    ...
    log_string(log_file, "EPE: %.4f, NEPE: %.4f, ACC3DS: %.4f, ACC3DR: %.4f, Outlier: %.4f, Dataset Size: %d" %
               (mean_epe,mean_nepe, mean_acc3d_strict, mean_acc3d_relax, mean_outlier, num_batch))
    ...
    if args.save_metrics:
        ...
        if args.use_test_time_refinement:
            ...
            data_for_save["div_flow_loss_refinement"] = div_flow_loss_refinement
    ...
    return mean_epe, mean_outlier, mean_acc3d_relax, mean_acc3d_strict,mean_nepe
...
def my_main(args):
    ...
    # Evaluation
    epsilon = 0.03 + torch.exp(model.epsilon).item()
    gamma = torch.exp(model.gamma).item()
    power = gamma / (gamma + epsilon)
    log_string(log_file, "Epsilon: %.4f, Power: %.4f" % (epsilon, power))
    eval_model(model, testloader, log_file, log_dir, res_dir, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, default="FluidFlow", help="Dataset. FlowNet3D_kitti or FlowNet3D_FT3D or Either HPLFlowNet_kitti or HPLFlowNet_FT3D.")
    ...
    parser.add_argument("--test_time_num_step", type=int, default=10, help="1: Number of steps for test time refinement.")
    parser.add_argument("--test_time_update_rate", type=float, default=0.01, help="1: Update rate for test time refinement.")
    ...
    parser.add_argument("--use_smooth_flow", type=int, default=0, help="1: Use self smooth flow loss in test time refinement, 0: Do not use smooth flow loss.")
    ...
    parser.add_argument("--use_div_flow", type=int, default=0, help="1: Use divergence flow loss in test time refinement, 0: Do not use divergence flow loss.")
    parser.add_argument("--nb_neigh_div_flow", type=int, default=1, help="Number of neighbor points for smooth flow loss in test time refinement.")
    parser.add_argument("--div_flow_loss_weight", type=float, default=0.01, help="Weight for divergence flow loss in test time refinement. Active if > 0.")
    ...
    parser.add_argument("--path2ckpt", type=str, default="", help="Path to saved checkpoint.")
    parser.add_argument("--log_fname", type=str, default="test_div_loss.txt", help="Evaluation log file name.")
