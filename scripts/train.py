...
def train(model, train_dataloader, val_dataloader, delta, optimizer, scheduler, path2log, args):
    ...
    for epoch in range(epoch_start, args.nb_epochs):
        ...
        if args.use_div_flow:
            running_div_flow_loss = 0
            training_div_flow_loss_tot = 0
        running_epe = 0
        training_epe_tot = 0
        epoch_it = 0
        ...
        for it, batch in enumerate(tqdm(train_dataloader)):
            ...
            # Run model
            recon_flow, corr_conf, target_recon, graph = model(batch["sequence"])
            

            # Compute loss
            loss, target_recon_loss, corr_conf_loss, smooth_flow_loss, div_flow_loss =\
            ...
            if args.use_div_flow:
                div_flow_loss_curr = div_flow_loss.item()
                running_div_flow_loss += div_flow_loss_curr
                training_div_flow_loss_tot += div_flow_loss_curr
            ...
            if it % delta == delta - 1:
                ...
                if args.use_div_flow:
                    writer.add_scalar("Loss/training_div_flow_loss", running_div_flow_loss / delta, total_it)
        ...
        if args.use_div_flow:
            training_div_flow_loss = training_div_flow_loss_tot / epoch_it
            writer.add_scalar("Loss/training_div_flow_loss", training_div_flow_loss, total_it)
        ...
        with torch.no_grad():
            for it, batch in enumerate(tqdm(val_dataloader)):
                ...
                if args.use_div_flow:
                    val_div_flow_loss_tot += div_flow_loss.item()
        ...
        if args.use_div_flow:
            val_div_flow_loss = val_div_flow_loss_tot / val_it
            writer.add_scalar("Loss/validation_div_flow_loss", val_div_flow_loss, total_it)
...
def my_main(args):
    ...
    args.nb_iter : int
        Number of unrolled iteration of Sinkhorn algorithm in MODEL.
    ...
    train(model, train_dataloader, val_dataloader, 500, optimizer, scheduler, path2log, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_div_flow", type=int, default=0, help="1: Use divergence flow loss, 0: Do not use divergence flow loss.")
    parser.add_argument("--div_flow_loss_weight", type=float, default=0.1, help="Weight for divergence flow loss. Active if > 0.")
    parser.add_argument("--div_neighbor", type=int, default=8, help="Number of neighbor points for divergence flow loss.")
    parser.add_argument("--lattice_steps", type=int, default=10, help="Number of lattice steps for divergence flow loss.")
    ...
    parser.add_argument("--log_dir", type=str, default="log_model", help="Logging directory.")
    ...
    args.use_div_flow = bool(args.use_div_flow)
